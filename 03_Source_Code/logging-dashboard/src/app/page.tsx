...
function buildMetrics(dashboard: AuditDashboard | null): Metrics {
  ...
  const lockedAccounts = [...new Set(items.filter((item) => item.event === 'LOCKED' && item.userId).map((item) => item.userId as string))];
...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-base)' }}>
      <Sidebar activeTab={activeTab} onTabChange={setActiveTab} lastRefreshLabel={lastRefreshLabel} onLogout={handleLogout} />
